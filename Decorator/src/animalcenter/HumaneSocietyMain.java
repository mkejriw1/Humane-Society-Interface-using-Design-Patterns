...
import decorations.BellAndTailDecorator;
import decorations.DressDecorator;
import decorations.FeatherDecorator;
...
public class HumaneSocietyMain {
	...
	public static void main(String[] args) {
		...
		statbutton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (human.animals.isEmpty()) {
					JFrame resultFrame = new JFrame("Statistic results");
					resultFrame.setVisible(true);
					resultFrame.setSize(500, 100);
					resultFrame.setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
					JPanel resultPanel = new JPanel();
					resultFrame.add(resultPanel);
					JLabel resultLabel = new JLabel(
							"Oops!!We currently do not have any animal friend with us!! Y don't you add one?");
		...
		addbutton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (finalType.equalsIgnoreCase("Dog")) {
					animals.add(
							new Dog(finalName, max, finalAge, finalType, finalBreed, adopt, finalMedical, finalNotes));
				} else if (finalType.equalsIgnoreCase("Cat")) {
					animals.add(
							new Cat(finalName, max, finalAge, finalType, finalBreed, adopt, finalMedical, finalNotes));
				} else if (finalType.equalsIgnoreCase("Chicken")) {
					animals.add(new Chicken(finalName, max, finalAge, finalType, finalBreed, adopt, finalMedical,
							finalNotes));
				} else if (finalType.equalsIgnoreCase("Sparrow")) {
					animals.add(new Sparrow(finalName, max, finalAge, finalType, finalBreed, adopt, finalMedical,
							finalNotes));
				}
				...
				add.dispose();
					...
					JFrame resultFrame = new JFrame("Age Not entered");
					resultFrame.setVisible(true);
					resultFrame.setSize(500, 100);
					resultFrame.setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
					JPanel resultPanel = new JPanel();
					resultFrame.add(resultPanel);
					JLabel resultLabel = new JLabel("Age entry is compulsory! Try Again ");
					resultPanel.add(resultLabel);
					add.dispose();
	...
	public void viewAnimal() {
		JFrame view = new JFrame("View An Animal");
		...
		JButton searchButton = new JButton("View Animal");
		...
		searchButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (animals.isEmpty()) {
					found = true;
					System.out.println("List is empty please add animal first");
				...
				if (!searchName.isEmpty()) {
					for (Animal ani : animals) {
						if (ani.getName().equalsIgnoreCase(searchName)) {
							found = true;
							System.out.println("Inside name found? " + found);
				...
				if (!found) {
					...
					JPanel resultPanel = new JPanel();
					resultFrame.add(resultPanel);
					JLabel resultLabel = new JLabel("Sorry no result found");
					resultPanel.add(resultLabel);
				}
				view.dispose();
			}
		...
		JButton tailDecorateButton = new JButton("View Animal with Tail & Belt decoration");
		panel.add(tailDecorateButton);
		tailDecorateButton.addActionListener(new ActionListener() {
				...
				if (!searchName.isEmpty()) {
					for (Animal ani : animals) {
						if (ani.getName().equalsIgnoreCase(searchName)) {
							...
							System.out.println("Inside name found? " + found);
							try {
								ani = new BellAndTailDecorator((Mammal) ani);
								ani.draw();
							} catch (java.lang.ClassCastException er) {
								JFrame resultFrame = new JFrame("Animal Cast Exception");
								resultFrame.setVisible(true);
								resultFrame.setSize(500, 100);
								resultFrame.setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
								JPanel resultPanel = new JPanel();
								resultFrame.add(resultPanel);
								JLabel resultLabel = new JLabel(
										"Honey! What you are trying to print is a Bird type not a Mammal !! Try Again ");
								resultPanel.add(resultLabel);
								view.dispose();
							}
		...
		JButton dressDecorateButton = new JButton("View Animal with Dress decoration");
		panel.add(dressDecorateButton);
		dressDecorateButton.addActionListener(new ActionListener() {
				...
				if (!searchName.isEmpty()) {
					for (Animal ani : animals) {
						if (ani.getName().equalsIgnoreCase(searchName)) {
							...
							System.out.println("Inside name found? " + found);
							try {
								ani = new DressDecorator((Mammal) ani);
								ani.draw();
							} catch (java.lang.ClassCastException er) {
								JFrame resultFrame = new JFrame("Animal Cast Exception");
								resultFrame.setVisible(true);
		...
		JButton newWingsButtons = new JButton("View Birds with another feather");
		panel.add(newWingsButtons);
		newWingsButtons.addActionListener(new ActionListener() {
				...
				if (!searchName.isEmpty()) {
					for (Animal ani : animals) {
						if (ani.getName().equalsIgnoreCase(searchName)) {
							...
							System.out.println("Inside name found? " + found);
							try {
								ani = new FeatherDecorator((Birds) ani);
								ani.draw();
							} catch (java.lang.ClassCastException er) {
								JFrame resultFrame = new JFrame("Animal Cast Exception");
								resultFrame.setVisible(true);
								...
								resultFrame.add(resultPanel);
								JLabel resultLabel = new JLabel(
										"Honey! What you are trying to print is a Mammal type not a Bird !! Try Again ");
								resultPanel.add(resultLabel);
								view.dispose();
							}
						} else {
		...
		for (Animal ani : animals) {
			if (typeCount.containsKey(ani.getType())) {
				...
				if (count.containsKey(ani.getBreed())) {
					...
					counter++;
					// System.out.println("inside if: "+ani.getType()+"
					// "+ani.getBreed()+" "+counter);
				} else {
					size++;
				}
